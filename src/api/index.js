...
function refreshTextPos(elem, initialLeft) {
    const elemParent = elem.parentElement;
    elem.remove();
    elem.style.left = initialLeft + "px";
    elemParent.append(elem);
}

function initTextScrollingAnimation(elemTarget, initialLeft, initialDistance, duration, isInfinite) {
    let startTime = null;
    ...
    const measure = (time) => {
        ...
        let shift = initialDistance * progress;
        elemTarget.style.left = (initialLeft - shift) + "px";
        ...
        if (progress < 1) {
            ...
            if (isInfinite) {
                refreshTextPos(elemTarget, initialLeft);
                startTime = null;
                requestId = requestAnimationFrame(measure);
            } else {
...
export const initScrollingText = (elem, duration, isInfinite = false) => {
    const elemParent = elem.parentElement;
    const initialLeft = elemParent.offsetWidth + 10; //adding some gap with 10px;

    refreshTextPos(elem, initialLeft);  //setting elem with text out of the visible bouderies of the parentElement
    const initialDistance = elem.offsetWidth + initialLeft;

    initTextScrollingAnimation(elem, initialLeft, initialDistance, duration, isInfinite);
};
